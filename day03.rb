...
class Grid
  ...
  def add(point)
    @points["#{point.x},#{point.y}".to_sym] = point
  end
...
class Point
  attr_reader :x
  attr_reader :y
  attr_reader :visited_by
  attr_reader :visited_after_steps
  ...
  def visit(path, steps)
    @visited_by << path
    @visited_after_steps[path] = steps
  end

  def intersection?
    @visited_by.uniq.count > 1
  end

  def steps_to_intersection
    @visited_after_steps.sum
  end
...
input.each_with_index do |path, index|
  ...
  path.each do |instruction|
    ...
    (1..distance).to_a.each do |_step|
      ...
      point = Grid.instance.get(current_x, current_y)
      point.visit(index, total_steps)
    end
  end
end

# Part 1
puts Grid.instance.all.values.select(&:intersection?).min_by(&:distance).distance

# Part 2
puts Grid.instance.all.values.select(&:intersection?).min_by(&:steps_to_intersection).steps_to_intersection
